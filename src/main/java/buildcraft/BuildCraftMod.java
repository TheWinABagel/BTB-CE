...
public class BuildCraftMod {
    ...
    class LocationSendRequest extends SendRequest {
        ...
        boolean isValid(EntityPlayer player) {
            return dimensionId == player.worldObj.provider.dimensionId && player.getDistanceSq(x, y, z) <= md;
        }
    }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    BCLog.logger
                            .error("The BuildCraft packet sender thread raised an exception! Please report to GitHub.");
                    e.printStackTrace();
                }
            }
        }
